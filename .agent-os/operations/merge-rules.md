...
- **무인 배치**: `codex/*` 구현 PR은 무인 리뷰-머지 루프(`jdsnack-review-merge-loop`)가 `code-reviewer` 5점 채점에서 **4점 이상이면 자동 머지**한다(4점 미만이면 구조화된 변경요청을 코덱스로). spec·거버넌스 PR은 사용자가 머지한다. **단, `High-risk` 변경(보안/외부 API/배포/DB/인증/CI)은 점수와 무관하게 자동 머지하지 않고 `needs-human`으로 사람에게 에스컬레이션한다.**
