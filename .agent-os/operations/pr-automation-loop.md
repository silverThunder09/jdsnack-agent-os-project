...
- Windows self-hosted runner의 PR feedback detector는 `C:\Program Files\Git\bin\bash.exe`를 명시적으로 사용하고 `.gitattributes`의 `*.sh text eol=lf`를 적용합니다. autonomous loop는 `JQ_BIN`·`GH_BIN`·`PYTHON_BIN`·`CLAUDE_BIN`·`CODEX_BIN`으로 런타임 의존성을 주입할 수 있으며, 누락 시 구조화된 `needs_human` 결과를 출력합니다.
