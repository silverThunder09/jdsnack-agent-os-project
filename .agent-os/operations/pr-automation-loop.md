...
- PR CI는 `.github/workflows/pr-ci-router.yml`이 변경 경로를 먼저 분류한 뒤 필요한 Backend/Frontend/Container/Docs workflow만 `workflow_call`로 실행합니다. Backend/Frontend/Container/Docs workflow는 PR에서 직접 전체 실행하지 않고, `main` push와 Router 호출을 기준으로 동작합니다.
