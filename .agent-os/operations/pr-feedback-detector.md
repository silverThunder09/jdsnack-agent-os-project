...
- 감지기는 polling loop를 내장하지 않습니다. `.github/workflows/pr-feedback-detector.yml`이 PR·리뷰·반려 Issue·CI 완료 이벤트마다 한 번 호출합니다.
...
- 현재 열린 이슈를 일괄 확인하려면 `scripts/open-issue-work-dispatcher.sh`를 한 번 실행하거나 `PR Feedback Detector` workflow를 `workflow_dispatch`로 실행합니다. `needs-human`·중복·미지원 이슈는 후보에서 제외합니다.
- `--apply`는 후보마다 최신 원격 브랜치에서 격리 worktree를 만들고 Codex 구현·테스트·커밋·푸시를 수행한 뒤 worktree를 제거합니다. PR 생성·머지는 수행하지 않습니다.
- Codex가 push 오류를 설명만 하고 성공 종료할 수 있으므로, runner가 push를 최대 3회 재시도하고 원격 SHA를 검증합니다. 그래도 실패하면 workflow는 성공으로 표시하지 않고 `needs_human`으로 중단합니다. job에는 30분 timeout이 있어 무한 대기를 방지합니다.
- 감지기는 GitHub 상태만 읽습니다. 코드 수정, 강제 push, PR 생성, 머지는 호출기가 별도 권한으로 수행합니다.
- `needs_human`은 자동으로 실패를 숨기지 않고 인증·권한·외부 상태 문제를 사람에게 올립니다. [Needs-human Discord 알림](needs-human-alerts.md)이 설정돼 있으면 발생원·브랜치·사유·링크만 한 번 전송합니다.
