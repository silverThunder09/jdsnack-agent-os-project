...
Codex와 서브 에이전트가 작업할 때 불필요한 파일을 읽지 않도록 탐색 표면을 줄입니다.  
...
- 하위 쓰레드는 과거 spec 전체가 아니라 handoff와 PR diff를 먼저 확인합니다.
...
- 계획 쓰레드: `AGENTS.md`, active spec, 제품/하네스 문서
- 개발 쓰레드: active spec, 담당 구현 폴더, 관련 테스트
- 검증 쓰레드: PR diff, 테스트 결과, handoff, 관련 spec
- 운영 쓰레드: 관련 workflow, script, 운영 문서

## 컨텍스트 압축 기준

- 읽은 파일이 많아지면 추가 탐색 전에 현재까지 확인한 사실을 요약합니다.
- 반복 질문이 나오면 새 탐색보다 기존 handoff와 traceability를 먼저 확인합니다.
