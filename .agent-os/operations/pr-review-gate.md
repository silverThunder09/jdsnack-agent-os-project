...
| 변경 범위 | 기본 리뷰 에이전트 |
|---|---|
| `.agent-os/specs/**`, API/UI 계약 | `Spec Steward` |
| `backend/**` | `Backend Engineer` |
| `frontend/**` | `Frontend Engineer` |
| 검증 쓰레드 | `QA Reviewer` |
| `.github/**`, CI/CD, Docker | `DevOps Steward` |
| 외부 API, 비밀값, 로그 정책 | `Security Reviewer` |
| PR/머지/릴리즈 판단 | `Release Captain` |

여러 범위가 섞여도 같은 쓰레드에서 모든 에이전트를 한 번에 합쳐 호출하지 않습니다.
Review Gate는 필요한 후보를 보여주되, 실제 리뷰는 handoff와 위험도에 따라 최소 에이전트만 수행합니다.
...
- 실제 호출 대상 에이전트의 `PASS` 또는 허용 가능한 `COMMENT`
- `REQUEST_CHANGES` 없음
