...
- AC-01: allowlist JobKorea 호스트(`www.jobkorea.co.kr`, `jobkorea.co.kr`) 요청은 200과 `sourceSite=jobkorea`, `fetchMode=static-html`, 요청 URL과 동일한 `sourceUrl`을 반환한다.
...
- AC-10: JobKorea URL 입력 분석 이력은 `jdInputType=JOBKOREA_URL`, `sourceSite=jobkorea`, 서버가 수집한 `sourceUrl`로 저장되고, 클라이언트가 다른 `sourceSite`를 보내도 서버 값이 저장된다. 기존 `SARAMIN_URL` 이력 조회는 그대로 동작한다.
- AC-11: 프론트는 JobKorea 호스트 링크에서 `JOBKOREA_URL`/`jobkorea`, 사람인 호스트에서 `SARAMIN_URL`/`saramin`, 링크 없음에서 `TEXT`/`null`을 전송하고, 지원 사이트 안내 문구에 JobKorea가 노출된다. 실패 안내는 error code별로 구분되고 상태를 색상만으로 구분하지 않는다.
