...
- REQ-10: JobKorea 본문 후보가 이미지 중심이거나 텍스트 추출 품질을 충족하지 못하면 source 전용 이미지 container의 `img[src]` 후보를 찾아 OCR fallback을 시도한다. 신뢰 가능한 JobKorea 이미지 호스트와 `http`/`https` scheme만 허용하고, 최대 이미지 8 MiB·redirect 3회·이미지 MIME 검사를 적용한다. OCR 성공 시 `sourceSite=jobkorea`, `fetchMode=image-ocr`, 요청 `sourceUrl`을 유지하며, OCR 불가·실패·품질 미달은 원래 fetch error와 기존 수동 입력 안내로 되돌린다. 기존 사람인 OCR selector·호스트·응답 계약은 유지한다.
