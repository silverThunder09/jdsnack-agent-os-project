...
- `sourceSite`는 `saramin` 또는 `jobkorea`이고, JobKorea 성공 응답의 `fetchMode`는 `static-html`이다. `sourceUrl`은 요청 URL을 그대로 보존한다.
...
- `inputType`이 `SARAMIN_URL` 또는 `JOBKOREA_URL`이고 `jd.text`가 비어 있으면 서버가 `jd.sourceUrl`로 `/api/jd/fetch` 경로와 같은 수집 로직을 실행한다.
