...
- **[개정 2026-06-19] `sentence` 옵션 라벨은 `문장 첨삭`으로 한다.** (이전 결정: "기존 문구 `맞춤 첨삭` 계열 유지" → 폐기.)
  - 근거: 옵션 라벨이 `맞춤 첨삭`이면 자기 결과 패널 제목(App.tsx 약 846행 `title="문장 첨삭"`)·내보내기 섹션(`buildResultMarkdown` 약 144행 `## 문장 첨삭`)·이 spec의 정식명("문장 첨삭")과 어긋난다. 같은 기능이 한 화면에서 두 이름으로 불리는 모순을 제거한다.
  - Codex 변경 지점: `frontend/src/App.tsx` `ANALYSIS_OPTIONS`의 `sentence` 항목(약 46행) `label: '맞춤 첨삭'` → `label: '문장 첨삭'`. 설명(`description`)에 `맞춤 첨삭` 문구가 있으면 결과물 명칭과 일치하도록 조정한다(설명 자체를 새로 만들지는 말 것).
...
- **[개정 2026-06-19] 사이드바 `맞춤 첨삭` 잠금 메뉴(`frontend/src/components/AppShell.tsx` `lockedItems`)를 제거한다.** (이전 결정: "별개 메뉴이므로 유지(해제 안 함)" → 폐기.)
- 근거(이전 "유지" 근거를 뒤집음):
  - 이전 근거는 사이드바 `맞춤 첨삭`을 분석 옵션 `sentence`와 무관한 "별개 미래 화면"으로 가정했다. 그러나 그 잠금 메뉴가 가리키던 기능이 바로 본 spec이 구현·머지한 "문장 첨삭(정식명: 문장 첨삭, 맞춤 첨삭·문장 단위)"이다.
  - 결과적으로 화면에 `맞춤 첨삭`이 ① 동작하는 분석 옵션과 ② 🔒잠긴(준비중처럼 보이는) 사이드바 항목으로 **두 번** 나타나는 중복이 발생한다. 이미 live인 기능을 🔒준비중으로 노출하는 것은 거짓 표시다.
  - 별도 "맞춤 첨삭 전용 화면" 계획은 로드맵에 없다(없는 기능을 잠금 메뉴로 예고하지 않는다).
- Codex 변경 지점: `frontend/src/components/AppShell.tsx` `lockedItems` 배열(약 13~20행)에서 `'맞춤 첨삭',` 항목(약 18행) **한 줄만 제거**한다. 다른 잠금 항목(`분석 내역`/`이력서 관리`/`템플릿`/`키워드 사전`/`요금제`)과 `lockedItems.map` 렌더 로직은 건드리지 않는다.
