...
- 모의 면접 질문 생성 기능을 추가: `POST /api/interview/preview`와 Stub/Fixture/Gemini 모드 분기, 프론트 질문 카드 섹션과 e2e 흐름
- PR 템플릿에 구현 기능·수행 테스트 상세 항목을 추가
- 모의 면접 spec을 archive로 이동하고 프론트 대시보드 전면 재설계를 활성 spec으로 전환

## 2026-06-13

- 표준·제품·운영 문서를 정리하고 Claude/Codex 분업 구성을 도입(`.claude` 하네스 추가, `.codex` 에이전트 정의 제거)
- 역할 경계를 명문화: CI/CD 배포는 코덱스 담당, 리뷰 통과 변경의 PR 마무리 커밋은 Claude 가능
- EC2 수동 배포와 운영 배포 검증 미실행 상태를 문서에 명시
- 변경 경로 기반으로 무거운 CI 단계를 건너뛰도록 최적화
- `pr-review-gate`를 범위 기반 리뷰로 정리하고 PR/이슈 템플릿을 정비
- 3단계 분석 UI에 맞춰 업로드→JD→리포트 e2e 시나리오를 갱신
- 모의 면접 질문 생성 활성 spec을 추가

## 2026-05-26

- 사람인 공고 본문 수집을 ajax/iframe 로드와 상세 직접 조회 fallback으로 보강
- 분석 화면을 SaaS형 3단계 흐름(업로드→JD→리포트)으로 재설계하고 JD 섹션 입력·비교 리포트 정보 구조를 정리
- 로컬 빌드용 compose와 배포 pull용 compose를 분리
- 로컬 `ai-local` 수동 검증 절차를 운영 문서로 정리
