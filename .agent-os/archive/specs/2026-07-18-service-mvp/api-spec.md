...
인증 상태 확인 API 자체는 공개 API다. 분석·이력 API는 이 응답의 `authenticated` 값을 프론트에서만 신뢰하지 않고, 서버의 공통 인증 경계를 통과해야 한다.
...
PDF/DOCX 업로드에서 분석 실패 이력까지 보존해야 하는 경우에는 다음 multipart 변형을 사용한다.

### `POST /api/analysis-histories/file`

`resumeFile` 파트와 `inputType`, `text`, `sourceUrl`, `sourceSite` 필드를 받아 기존 텍스트 추출 경계로 이력서 본문을 추출한 뒤 동일한 분석 이력 생성 흐름을 실행한다. 원본 파일은 저장하지 않는다. `inputType`은 `TEXT` 또는 `SARAMIN_URL`이다.
