...
export function AnalysisPanel({ badge, title, description, result, resultRef, successContent }: { badge: string; title: string; description: string; result: ResultState; resultRef?: RefObject<HTMLElement | null>; successContent: ReactNode }) {
  return <section className="preview-panel" aria-live="polite" ref={resultRef} tabIndex={resultRef ? -1 : undefined}>
    ...
    {result.status === 'idle' ? <StatusMessage badge="준비" title="아직 결과가 없습니다" message={description} tone="neutral" /> : null}
    {result.status === 'loading' ? <StatusMessage badge="분석 중" title={result.title} message={result.message} tone="active" withLoadingBar /> : null}
    {result.status === 'not-enabled' ? <StatusMessage badge="준비 중" title={result.title} message={result.message} tone="success" /> : null}
    {result.status === 'error' ? <StatusMessage badge="확인 필요" title={result.title} message={result.message} tone="danger" /> : null}
...
export function ComingSoonPanel({ title, description }: { title: string; description: string }) {
  return <section className="preview-panel"><div className="preview-panel__header"><span>준비중</span><div><h2>{title}</h2><p>{description}</p></div></div><StatusMessage badge="준비 중" title="준비 중인 분석입니다" message="이 분석 항목은 현재 준비 중입니다. 곧 제공될 예정이에요." tone="neutral" /></section>
}
