  ...
  await page.route('**/api/auth/session', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        ...
        data: {
          authenticated: true,
          user: { id: 'e2e-user', email: 'e2e@example.com', displayName: 'E2E 사용자' },
        },
      }),
    })
  })

  await page.route('**/api/analysis-histories', async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          data: {
            ...
            input: { resumeText: 'resume', jdInputType: 'TEXT', jdText, sourceUrl: null, sourceSite: null },
...
test('키워드 분석만 선택해도 매칭 요청과 키워드 결과를 제공한다', async ({ page }) => {
  ...
  await page.getByRole('checkbox', { name: /문장 첨삭/ }).uncheck()
  await page.getByRole('button', { name: '분석 시작하기 →' }).click()
...
test('문장 첨삭만 선택해도 독립 호출과 Before→After 결과를 제공한다', async ({ page }) => {
  await mockFixtureFlow(page)
  await page.goto('/')
