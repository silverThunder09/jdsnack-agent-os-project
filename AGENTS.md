...
JDSnack은 개발자 이력서와 JD를 AI로 분석하는 웹 서비스입니다. 활성/대기 기획은 `.agent-os/standards/index.yml`의 `active_specs`(구현 대상 1개)·`pending_specs`(대기 큐)로 관리하고, 과거 기획 문서는 archive에 보관합니다.
...
- 코덱스: 활성 spec 기준 기능 **구현 + 기능 테스트 작성**, 리뷰 기반 코드 수정·커밋·푸시, CI/CD 배포(사용자 지시 시).
- 폴백: 코덱스가 토큰이 없어 막힐 때만, 사용자가 "네가 구현해"라고 지시하면 클로드가 직접 구현·테스트합니다.
- **무인 배치**: 코덱스가 `codex/*` 브랜치로 push하면 **무인 리뷰-머지 루프(`jdsnack-review-merge-loop`, cron)가 자체 세션에서 리뷰(`code-reviewer` 5점 채점)→PR→4점 이상이면 자동 머지**합니다. spec PR·거버넌스 PR은 사용자가 머지합니다.
...
5. 현재 활성/대기 기능: `.agent-os/standards/index.yml`의 `active_specs`(구현 대상 1개)·`pending_specs`(대기 큐) 참조
