...
JDSnack은 개발자 이력서와 JD를 AI로 분석하는 웹 서비스입니다. 현재 활성 기획은 키워드 매칭 구조화 / 키워드 분석 옵션 해금이며, 과거 기획 문서는 archive에 보관합니다.
...
- CI/CD 배포(GHCR 이미지 publish, `compose.prod.yaml`, 배포 워크플로/런북, 자동 배포·배포 검증)는 코덱스가 담당하며, 사용자가 지시할 때 실행합니다.
- 단, PR 검증용 CI 체크(테스트/빌드 게이트, 경로 필터 등)의 운영·효율화는 클로드의 PR/merge 운영 범위입니다.
...
5. 현재 활성 기능: `.agent-os/specs/2026-06-17-keyword-matching-breakdown/`
